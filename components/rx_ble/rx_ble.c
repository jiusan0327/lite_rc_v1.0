...
#define SAMPLE_DEVICE_NAME          "rx_litrmt"    //The Device Name Characteristics in GAP
...
static const uint8_t spp_adv_data[23] = {
    ...
    0x0F,0x09, 'R', 'X', 'J', 'Q', 'R', '-', 'L', 'I', 'T', 'E', 'R','M', 'T', '1'
};
...
#endif
uint8_t cmd_temp[SPP_CMD_MAX_LEN] = {0x00};
...
void spp_cmd_task(void * arg)
{
    ...
    for(;;){
        ...
        if(xQueueReceive(cmd_cmd_queue, &cmd_id, portMAX_DELAY)) {
            cmd_temp[0] = cmd_id[0];
            ESP_LOGI(GATTS_TABLE_TAG, "Received command: %s", cmd_id);
            free(cmd_id);
        }
    }
    vTaskDelete(NULL);
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	...
    	case ESP_GATTS_DISCONNECT_EVT:
            ESP_LOGI(GATTS_TABLE_TAG, "Disconnected, reason: 0x%x", p_data->disconnect.reason);
            spp_mtu_size = 23;
    	    is_connected = false;
    	    enable_data_ntf = false;
#ifdef SUPPORT_HEARTBEAT
...
void rx_spp_ble_init(void)
{
    ...
    spp_task_init();
    // 添加安全配置
    esp_ble_auth_req_t auth_req = ESP_LE_AUTH_REQ_SC_MITM_BOND;  // 安全连接 + MITM保护 + 绑定
    esp_ble_io_cap_t iocap = ESP_IO_CAP_NONE;                    // 设备无输入输出能力
